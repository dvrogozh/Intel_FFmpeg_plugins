                                 ...
                                 av_unused uint32_t       size)
{
    ...
    VAAPIDecodePicture *pic = s->curframe->hwaccel_picture_private;
    VAPictureParameterBufferVP8 pic_param, *pp;
    VAProbabilityDataBufferVP8 prob;
    VAIQMatrixBufferVP8 quant;
    int i, j, k, err;

    pic->output_surface = ff_vaapi_get_surface_id(s->curframe->tf.f);
    pp = &pic_param;
    pp->frame_width                     = avctx->width;
    pp->frame_height                    = avctx->height;

    pp->last_ref_frame                  = vaapi_vp8_surface_id(s->framep[VP56_FRAME_PREVIOUS]);
    pp->golden_ref_frame                = vaapi_vp8_surface_id(s->framep[VP56_FRAME_GOLDEN]);
    pp->alt_ref_frame                   = vaapi_vp8_surface_id(s->framep[VP56_FRAME_GOLDEN2]);
    pp->out_of_loop_frame               = VA_INVALID_SURFACE;

    pp->pic_fields.bits.key_frame                   = !s->keyframe;
    pp->pic_fields.bits.version                     = s->profile;

    pp->pic_fields.bits.segmentation_enabled        = s->segmentation.enabled;
    pp->pic_fields.bits.update_mb_segmentation_map  = s->segmentation.update_map;
    pp->pic_fields.bits.update_segment_feature_data = s->segmentation.update_feature_data;

    pp->pic_fields.bits.filter_type                 = s->filter.simple;
    pp->pic_fields.bits.sharpness_level             = s->filter.sharpness;

    pp->pic_fields.bits.loop_filter_adj_enable      = s->lf_delta.enabled;
    pp->pic_fields.bits.mode_ref_lf_delta_update    = s->lf_delta.update;

    pp->pic_fields.bits.sign_bias_golden            = s->sign_bias[VP56_FRAME_GOLDEN];
    pp->pic_fields.bits.sign_bias_alternate         = s->sign_bias[VP56_FRAME_GOLDEN2];

    pp->pic_fields.bits.mb_no_coeff_skip            = s->mbskip_enabled;
    pp->pic_fields.bits.loop_filter_disable         = s->filter.level == 0;

    pp->prob_skip_false                 = s->prob->mbskip;
    pp->prob_intra                      = s->prob->intra;
    pp->prob_last                       = s->prob->last;
    pp->prob_gf                         = s->prob->golden;
                                  ...
                                  uint32_t        size)
{
    ...
    slice_param = (VASliceParameterBufferVP8) {
        .slice_data_size   = size - header_size,
#ifdef VPG_DRIVER
    ...
    slice_param.partition_size[0] = s->header_partition_size - ((slice_param.macroblock_offset + 7) / 8);
    for (i = 0; i < 8; i++)
        slice_param.partition_size[i+1] = s->coeff_partition_size[i];
